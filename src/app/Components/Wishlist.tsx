...
const Wishlist = () => {
  ...
  return (
    <div className="flex justify-center items-center flex-col relative top-[30px]">
      {wishlist.length === 0 ? (
        ...
        <div className="flex justify-center items-center flex-col gap-y-5">
          ...
          <p className="text-white/70 text-[14px] font-roboto font-normal w-[800px] text-center">
            At ChowChampl, we know how hard it is to pick just one dish.
            Now, with our wishlist feature, you can save your favorite menu
            items and come back to them whenever you're ready to indulge.
            From mouth-watering appetizers to delectable desserts, your next
      ...
      <h2 className="text-white font-bold font-roboto text-[14px]">Image</h2>
      <h2 className="text-white font-bold font-roboto text-[14px]">Product</h2>
      <h2 className="text-white font-bold font-roboto text-[14px]">Amount</h2>
      <h2 className="text-white font-bold font-roboto text-[14px]">Status</h2>
      <h2 className="text-white font-bold font-roboto text-[14px]">Action</h2>
      <h2 className="text-white font-bold font-roboto text-[14px]">Remove</h2>
    </div>
  </div>
  {wishlist.map((item: any) => (
   ...
   <h2 className="w-[200px] text-white font-medium  font-roboto text-[14px]">{item.name}</h2>
   <span className="">{item.discount}</span>
   </div>
      <p className="w-[100px] relative left-[-25px]">${item.price}</p>
      <p className={`w-[100px] h-[20px] bg-yellow-500 relative left-[-35px] ${item.stock === 'In Stock' ? 'text-green-500' : 'text-red-500'}`}>{item.stock}</p>
      <button className={`px-4 py-2 rounded text-white ${item.stock === 'In Stock' ? 'bg-btnBackground' : 'bg-gray-400 cursor-not-allowed'}`} disabled={item.stock !== 'In Stock'}>
